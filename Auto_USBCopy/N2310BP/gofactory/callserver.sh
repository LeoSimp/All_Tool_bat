...
if [ $? -eq 0 ]; then
        # This is not a NAS, rename the MODELNAME
        MODELNAME=`awk '/type/{print $2}' /etc/manifest.txt`
fi

MBTYPE=`awk '/^MBTYPE/{print $2}' /proc/thecus_io`
MBTYPE_MAJOR=`echo $MBTYPE | head -c 2`

export PATH=$PATH:/opt/bin:/usr/bin:/etc/gofactory
WGET="wget"
HEXDUMP="hexdump"
...
function genrandkey() {
  ...
  ${HEXDUMP} $filerand | head -1|awk '{printf("%s%s%s%s%s",$2,$3,$4,$5,$6)}'
}
...
function report2server() {
  ...
  postdata="type=$TYPE&targetmac=$MAC1$mac2data$mac3data&targetip=$WANIP&sessionkey=$SESSIONKEY&test_version=$FACTORY_VERSION&fw_version=$FW_VERSION&bios_version=$BIOS_VERSION&lcd_version=$LCD_VERSION&model=$MMODEL_enc&producer=$MPRODUCER&testid=$TESTID&result=$RESULT&message=$MESSAGE$exceptionlist&usbid=$USBID"
