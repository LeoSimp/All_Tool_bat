...
update_nic2() {
  ...
  if [ "$nic2ip" != "" ];then
    ifconfig $nic2interface up
    ifconfig $nic2interface $nic2ip $NETMASK $BROADCAST
  fi
}

update_nic3() {
  nic3interface="eth2"
  nic3ip=`echo "$ip"|awk -F'.' '{ip1=$1;printf("%d.%d.%d.%d",ip1+2,$2,$3,$4)}'`
  if [ "$nic3ip" != "" ];then
    ifconfig $nic3interface up
    ifconfig $nic3interface $nic3ip $NETMASK $BROADCAST
  fi
}
